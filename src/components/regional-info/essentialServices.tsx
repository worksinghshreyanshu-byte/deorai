  ...
  return (
    <div className="rounded-3xl border border-[#D7C3AC] bg-[#F8F1E8] p-5 lg:p-8">
      <div className="flex items-center gap-4">
        ...
        <h3 className="font-serif lg:text-2xl text-lg font-semibold text-brand">
          {title}
        </h3>
      </div>
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex justify-between gap-3 lg:gap-4">
              <h4 className="font-semibold  text-sm text-[#5E4634]">
                {item.name}
              </h4>

              <span className="rounded-full text-center bg-[#F3E7D8] px-3 py-1 lg:text-sm text-xs font-medium text-brand">
                {item.distanceKm} km
              </span>
            </div>
